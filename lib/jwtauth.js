  ...
  var jwtauth = {
    auth: function(req, res, next) {
      ...
      try {
        ...
        console.log('TRY jwtTokenSecret is ' + decoded);
      } catch (err) {
        console.log('CATCH jwtTokenSecret is ' + decoded);
        return res.status(401).json({'msg':'!decoded access denied'});
      }

      User.findOne({'_id': decoded.iss}, function(err, user) {
        if(err) return res.status(500).json(err);

        if(!user) return res.status(401).json({'msg': '!user access denied'});
